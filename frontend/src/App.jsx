...
function HomePage({ shop, queue, connected, navigate }) {
  return <main className="home-page">
    ...
    <section className="content-section" id="servicos">
      <div className="section-title"><p className="eyebrow">SERVIÇOS</p><h2>Escolha seu estilo.</h2><p>Preço e duração sem surpresa.</p></div>
    ...
    <section className="dark-section"><p className="eyebrow">COMO FUNCIONA</p><h2>Entrou.<br/>Acompanhou.<br/>Cortou.</h2><div className="steps"><div><span>1</span><p>Entre na fila.</p></div><div><span>2</span><p>Veja sua posição.</p></div><div><span>3</span><p>Chegue perto da vez.</p></div></div><button className="big-action light" onClick={() => navigate('fila')}>VER FILA <Icon name="arrow"/></button></section>
...
function QueuePage({ shop, queue, setQueue, connected }) {
  ...
  return <main className="queue-page">
    <section className="queue-hero"><p className="eyebrow">FILA DO {shop.name.toUpperCase()}</p><h1>Sua vez,<br/>sem espera.</h1><p>Acompanhe a ordem em tempo real.</p><StatusPill queue={queue} connected={connected}/></section>
...
function GalleryPage({ shop }) {
  ...
  return <main className="gallery-page"><section className="gallery-heading"><p className="eyebrow">CORTES DO DENIS</p><h1>Escolha sua<br/>próxima versão.</h1><p>Toque em uma foto para ampliar.</p></section><section className="gallery-grid">{shop.portfolio.map((item, index) => <button key={item.id} className={`gallery-item item-${index + 1}`} onClick={() => setSelected(item)}><img src={item.image} alt="" style={{ objectPosition: item.position }} onError={event => { event.currentTarget.hidden = true; event.currentTarget.closest('.gallery-item')?.classList.add('missing-photo') }}/><div><small>{item.category}</small><strong>{item.title}</strong></div></button>)}</section><section className="gallery-cta"><p>Gostou de algum?</p><a href={`https://wa.me/${shop.whatsapp}?text=${encodeURIComponent('Olá! Vi os cortes no site e quero conversar sobre o meu.')}`} target="_blank" rel="noreferrer">FALAR COM O DENIS <Icon name="arrow"/></a></section>{selected && <div className="photo-modal" onClick={() => setSelected(null)}><button aria-label="Fechar">×</button><img src={selected.image} alt={selected.title}/><div><small>{selected.category}</small><strong>{selected.title}</strong></div></div>}</main>
}
